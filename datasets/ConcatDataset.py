...
from utils import printlog, time_funcall
import torch
import datetime

# from omnivision.data.omni_dataset import OmniDataset
try:
    from .omnivision import DefaultOmnivoreCollator
except ImportError:
    from omnivision import DefaultOmnivoreCollator
...
class ConcatIterator:
    def __init__(self, concat_dataset, **kwargs) -> None:
        ...
        # tp: gives the number of batches in each Dataloader
        ...
        # repetition = -1
        max_iterator_len = max(iterator_lens)
        # set the iterator length to max length if rf == -1
        for idx in range(len(concat_dataset.repeat_factors)):
            if concat_dataset.repeat_factors[idx] == -1:
                iterator_lens[idx] = max_iterator_len
        ...
        if concat_dataset.batching_strategy == "use_all":
            ...
            # tp: indices' elements are in range {0,..., num_data_sources-1}
            indices = []
            for idx in range(num_data_sources):
        ...
        # logging.info(
        printlog(
            f"Created a ConcatIterator with batching_strategy={concat_dataset.batching_strategy} "
            f"and steps={concat_dataset.max_steps}. Steps per source:"
        )
        for idx in range(num_data_sources):
            assert (
            ...
            if concat_dataset.repeat_factors[idx] == -1:
                ...
                assert self.source_to_use[idx].sum().item() == self.max_steps,\
                    f"{self.source_to_use[idx].sum().item()} != {self.max_steps}"
            else:
                assert self.source_to_use[idx].sum().item() == int(
                    len(dataloaders[idx]) * concat_dataset.repeat_factors[idx]
                )
            # logging.info(
            printlog(f"Epoch: {epoch} - Dataset {idx}; len {iterator_lens[idx]}; Orig len {len(dataloaders[idx])}")
    ...
    def get_sample(self):
        ...
        for idx in range(len(self.iterators)):
            ...
            try:
                ...
                printlog(f"Reached end of iterator {idx} at step {self.step_counter}")
            ...
            if self.per_src_step_counter[idx] > self.iterator_lens[idx]:
                raise ValueError(f"Something is off. For iterator {idx} expected {self.iterator_lens[idx]} "
                                 f"steps but currently at {self.per_src_step_counter[idx]}")
    ...
    def __next__(self):
        ...
        # timed_get_sample = time_funcall(self.get_sample)
        # sample = timed_get_sample()
...
class DatasetDataloader(Dataset):
    ...
    def get_loader(self, epoch) -> Iterable:
        ...
        train_loader = DataLoader(self.dataset,
                                  ...
                                  collate_fn=DefaultOmnivoreCollator(output_key=self.output_key),
                                  drop_last=True,
                                  pin_memory=True,
